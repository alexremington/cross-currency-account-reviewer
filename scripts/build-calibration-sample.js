...
function compactLedger(row) { const aliases = { leftId: ['leftId', 'leftRecordId', 'left_record_id'], rightId: ['rightId', 'rightRecordId', 'right_record_id'], score: ['score', 'pairScore', 'pair_score'], band: ['band', 'lane'], lane: ['lane', 'band'], exactIdentity: ['exactIdentity', 'exact_identity'], modelVersion: ['modelVersion', 'model_version'], nameStatus: ['nameStatus', 'name_status'], websiteStatus: ['websiteStatus', 'website_status'], phoneStatus: ['phoneStatus', 'phone_status'], billingAddressStatus: ['billingAddressStatus', 'billing_address_status'], ultimateParentAccountStatus: ['ultimateParentAccountStatus', 'ultimate_parent_account_status'], fieldScores: ['fieldScores', 'field_score_summary'], reasonCodes: ['reasonCodes', 'reasons'], accountNameRelationship: ['accountNameRelationship', 'account_name_relationship'], contradictionCategory: ['contradictionCategory', 'contradiction_category'] }; const result = {}; for (const [key, keys] of Object.entries(aliases)) result[key.toLowerCase()] = value(row, ...keys); return result; }
...
function semanticCase(left, right, ledger, features, seed, index) {
  ...
  return {
    ...
    fieldScores: tryJson(value(ledger, 'fieldScores', 'fieldscores'), {}), reasonCodes: String(value(ledger, 'reasonCodes', 'reasoncodes')).split(' | ').filter(Boolean),
