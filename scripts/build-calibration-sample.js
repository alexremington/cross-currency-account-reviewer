...
function classify(left, right, ledger, crossPair) {
  ...
  const producerScore = value(ledger, 'score', 'operationalScore', 'operationalscore', 'pairScore', 'pair_score');
  const score = Number(crossPair?.score ?? (producerScore || 0));
...
function semanticCase(left, right, ledger, crossPair, features, seed, index) {
  ...
  return {
    caseId, stratum: features.stratum, score: crossPair.score, band: crossPair.band, lane: crossPair.lane,
    exactIdentity: Boolean(crossPair.exactIdentity), modelVersion: String(crossPair.modelVersion || ACCOUNT_MODEL_VERSION),
    producerScore: Number(value(ledger, 'score', 'operationalScore', 'operationalscore', 'pairScore', 'pair_score') || 0),
    evidenceStatuses: Object.fromEntries(crossPair.evidence.map((item) => [item.field === 'address' ? 'billingAddress' : item.field === 'ultimate_parent_account__c' ? 'ultimateParentAccount' : item.field, item.status])),
    fieldScores: crossPair.fieldScores || {}, evidenceFields: crossPair.evidence.filter((item) => item.status === 'matched').map((item) => item.field), reasonCodes: crossPair.reasonCodes || [],
    exactConfidenceRule: String(crossPair.exactConfidenceRule || ''), exactConfidenceStatus: crossPair.exactConfidenceEligible ? 'eligible' : '',
    accountNameRelationship: String(crossPair.accountNameRelationship || ''), contradictionCategory: String(crossPair.contradictionCategory || ''),
...
const fallbackBuckets = new Map(Object.keys(DEFAULT_QUOTAS).map((key) => [key, []]));
const seen = Object.fromEntries(Object.keys(DEFAULT_QUOTAS).map((key) => [key, 0]));
const fallbackSeen = Object.fromEntries(Object.keys(DEFAULT_QUOTAS).map((key) => [key, 0]));
function offer(stratum, candidate, quota, fallback = false) {
  const limit = Math.max(0, number(quota, 0)); if (!limit) return;
  const counts = fallback ? fallbackSeen : seen;
  const target = fallback ? fallbackBuckets : buckets;
  counts[stratum] += 1; const bucket = target.get(stratum); if (bucket.length < limit) { bucket.push(candidate); return; }
  const replacement = Math.floor(rng() * counts[stratum]); if (replacement < limit) bucket[replacement] = candidate;
}
...
for await (const ledger of csvObjects(ledgerPath, compactLedger)) {
  ...
  const currenciesDiffer = normalize(left.currencyisocode) && normalize(right.currencyisocode) && normalize(left.currencyisocode) !== normalize(right.currencyisocode);
  if (!currenciesDiffer) continue;
  const crossPair = scorePair(left, right);
  candidateCount += 1; const candidate = { left, right, ledger, crossPair, features: classify(left, right, ledger, crossPair), index };
  const isLowScoreControl = candidate.features.score < 30;
  offer(candidate.features.stratum, candidate, quotas[candidate.features.stratum], isLowScoreControl);
  offer('randomControls', candidate, quotas.randomControls, isLowScoreControl);
}
const selected = []; const counts = {};
const fallbackCounts = {};
for (const [stratum] of Object.entries(quotas)) {
  const limit = Math.max(0, number(quotas[stratum], 0));
  const preferred = buckets.get(stratum) || [];
  const fallback = fallbackBuckets.get(stratum) || [];
  const take = [...preferred, ...fallback].slice(0, limit).map((candidate) => stratum === 'randomControls' ? { ...candidate, features: { ...candidate.features, stratum } } : candidate);
  counts[stratum] = take.length;
  fallbackCounts[stratum] = Math.max(0, take.length - preferred.length);
  selected.push(...take);
}
const rawCases = selected.map((candidate, index) => ({ caseId: `case-${hash(`${seed}|${value(candidate.ledger, 'leftId', 'leftid')}|${value(candidate.ledger, 'rightId', 'rightid')}`)}`, left: candidate.left, right: candidate.right, ledger: candidate.ledger, crossPair: candidate.crossPair, features: candidate.features, selectionIndex: index }));
const semanticCases = selected.map((candidate, index) => semanticCase(candidate.left, candidate.right, candidate.ledger, candidate.crossPair, candidate.features, seed, index));
const metadata = { calibrationVersion: CALIBRATION_VERSION, generatedAt: new Date().toISOString(), sourceArtifact: sourcePath, ledgerArtifact: ledgerPath, producerVersion: arg('producer-version', ''), modelVersion: ACCOUNT_MODEL_VERSION, adapterVersion: 'cross-currency-policy-adapter/v1', selectionPolicy: 'prefer Cross Currency scores >= 30; retain lower scores only when a required stratum lacks enough preferred candidates', seed, quotas, selectionCounts: counts, fallbackSelectionCounts: fallbackCounts, candidateCount, selectedCount: selected.length };
